...
def main():
    ...
    if input("Do you want to test projects (y/n): ").lower().strip() in ("yes", "y"):
        ...
        for i, j in resp.items():
            print(
                f"Project `{i}` passed {sum(k[0] for k in j)}/{len(j)} Tests: {''.join([f'{Fore.RED}X', f'{Fore.GREEN if autograder.in_replit() else Fore.BLUE}✓'][m[0]] for m in j)}{Style.RESET_ALL}"
            )
